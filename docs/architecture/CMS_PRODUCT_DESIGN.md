...
**Status:** Implemented. The five open decisions in Appendix B were analyzed (problem, options, trade-offs, recommendation) and resolved to the recommended default below, and Studio now ships the design described here. This document remains the canonical reference cited throughout `src/app/studio` and `src/components/studio`.
...
**Updated (Experience v3 Preview Integrity milestone) — supersedes the original design below.** The editor's own canvas never doubled as a public-accuracy guarantee in practice: `BlockRenderer` (Studio-only, Tailwind admin chrome) and `ProseRenderer` (public site, `.public-*` CSS) are, and always ended up being, genuinely separate renderers, so "toggle the canvas into `BlockRenderer`" never actually proved what a visitor would see. The real mechanism is now Next.js Draft Mode: a **Preview** action (toolbar, or next to the view page's Edit button) flushes any unsaved autosave, then opens the entity's real public URL — the actual route, through the actual `ProseRenderer`/`PublicCollectionDetail`, fed the unpublished record only for that signed-in, authorized session (`/api/preview` enables Draft Mode after re-checking the same `canActOnEntry` gate Studio itself uses; `/api/preview/exit` clears it). A site-wide banner marks the page as a preview. This is what makes "what I'm about to publish" and "what ships" the same guarantee — proven by literally being the same render path, not a resemblance of it.

### Publishing workflow

The status stepper in the header shows only the transition(s) valid for the acting role (§28/§29/§30) — never a five-option dropdown. **Updated (Experience v3 Publishing Integrity milestone):** when an entry is sent back from In Review to Draft, the reviewer must attach a note explaining why — an empty note is rejected server-side, not merely encouraged. This closed what shipped for a time as an optional, skippable field (see Appendix B, decision 3's correction below).

**Superseded (Appendix B, decision 1) — the original "goes live immediately" resolution below shipped, then was reopened and closed the other way.** Editing an already-**Published** entry now forces it back to `inReview` (clearing any stale reviewer note) — no opt-in step, so it can't be silently skipped. The trade-off this accepts, stated plainly: the edited content temporarily disappears from the public site until it's re-approved and re-published. This isn't the draft/live dual-copy versioning §36 still reserves for later — it's the smaller fix of closing the gap where a published entry's owner (or any Admin) could silently rewrite it with zero re-review, which the original resolution below left open.
...
2. Reviews the Case Study document via **Preview** — the real public route, rendered through Draft Mode (see §5's updated Preview section).
3. Either **Approves** → **Publishes** (triggers on-demand ISR revalidation, §21/§28), or sends it back to Draft with a required note — the author sees that note the next time they open the entry.
...
1. **Editing a Published entry** — **Superseded.** Originally resolved as "goes live immediately, no forced re-review." That shipped, then was identified as leaving publishing accountability open (any edit to a live entry bypassed review entirely) and was closed the other way in the Experience v3 Publishing Integrity milestone: editing a Published entry now forces it back to `inReview`. See §5's updated Publishing workflow section.
...
3. **Reviewer rejection notes** — **Superseded.** Originally resolved as optional/skippable. Shipped as **required** instead in the Experience v3 Publishing Integrity milestone — an empty note is rejected server-side. See §5.
