...
>
> **Amended.** §1 and §5 point 1's call to retire electric blue outright is superseded — `11_COLOR_PHILOSOPHY_AMENDMENT.md` re-derives the same blue from the studio's own existing mark and real material references instead, on the finding that the original problem was absent derivation and restraint, not the hue itself. §4's "one signature interactive accent" should be read as "one signature _interactive_ color, plus a related technical-diagram register drawn from the same tonal family" — `11_COLOR_PHILOSOPHY_AMENDMENT.md` §6 owns that second register (Ice Blue) as part of one layered palette rather than an unacknowledged exception. §8's Recognition Test gains a concrete, falsifiable sibling for the brand mark specifically — `13_BRAND_SYSTEM.md` §1's Favicon Test. `16_SIGNATURE_MOMENTS.md` names five specific, worked interactions (not one per pillar) that give this document's "would a reader remember this tomorrow" test (§3) concrete, buildable answers rather than a standard with no worked example.
