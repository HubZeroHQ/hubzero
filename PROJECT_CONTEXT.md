...
| `05_CONTENT_STRATEGY.md`            | Content types and policy — case-study bar, zero-tolerance testimonial/stat fabrication policy, blog scope, FAQ content, multi-language (English-only).                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
**Practical reading order for a new session:** `00` (what changed and why) → `14` (what phase you're in) → the specific numbered doc for whatever page/feature you're touching → `15`/`16` if you're touching visual/responsive work on any marketing page.
...
- **Unified rendering pipeline for any markdown/rich-text content** (once the blog/CMS exists) — CMS preview and published output must use the exact same pipeline; this directly fixes a confirmed legacy bug where preview and published output visually diverged.
...
- `/blog`, `/blog/[slug]`, `/careers`, `/privacy`, `/terms` — all unbuilt.
...
5. **`/blog` + `/blog/[slug]`** — full platform per `ARCHITECTURE/05` §5 / `06`, with 3–5 cornerstone launch articles.
...
- **Blog ships as a complete platform in the MVP**, not deferred — reverses the original team-consensus reading of the survey data. Launch content is only 3–5 cornerstone articles, but the architecture must be production-ready at launch.
...
8. **Check §14 (Known Issues) and §15 (Pending Roadmap) above** before assuming a page or system exists — several core pages (Contact, Team, Blog) and the entire backend/CMS are not built yet despite several public pages already existing.
