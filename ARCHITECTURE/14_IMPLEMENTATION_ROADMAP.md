...
- **[New, 2026-07-04]** The four-pillar company structure (`17_COMPANY_STRUCTURE.md`) is real net-new scope layered on a roadmap that, as of this amendment, hasn't yet shipped Contact, the CMS, or Team/Blog/Careers. Weighed the same way the blog-MVP decision was weighed above: Labs ships alongside the other Phase 3 pages because it is substantially a *migration* of already-existing content (the IoT Sensor Dashboard currently on the Hardware page), not new-content creation — low cost, in scope. Builds and Blueprints are each gated on real content existing (a finished internal product; a Blueprint with a working live demo) and are treated as fast-follow candidates, same tier as blog search/RSS refinements, unless that content already exists by the time Phase 3 begins — see Phase 3a below.
...
## Phase 4 — Blog (full platform, shipped in the MVP) **[Amended 2026-07-01, see `00_FOUNDER_APPROVAL.md` §3]**

The original CSV-consensus reading ("maybe in the future," not required for launch) is superseded: the **complete blog platform ships in the MVP** — public index/post templates, unified rendering pipeline, categories, tags, SEO, search, RSS, and author profiles — populated at launch with **3-5 cornerstone articles** that demonstrate real expertise. This is a genuine scope addition against the original "infrastructure now, content later" sequencing, and should be weighed against the Phase 0 timeline constraint (§0 above) — if the 3-month bar is at risk, blog search/RSS refinements are the first candidate to push into a fast-follow release, since the core CMS collection and basic templates are the part every other phase already assumes exists.
...
| Advanced blog search, RSS refinements, recommendation engine, advanced analytics, richer editorial tooling | Core blog platform and basic CMS/RBAC ship in the 3-month MVP; these are the explicit fast-follow candidates if the deadline is at risk | `00_FOUNDER_APPROVAL.md` §1 |
...
1. Every page in `03_INFORMATION_ARCHITECTURE.md` §1 exists, is reachable, and contains real (not placeholder) content, including the migrated Labs entry (`/labs/[slug]`) and 3-5 cornerstone blog articles. **[Amended 2026-07-04]** Builds and Blueprints are conditional on Phase 3a: required for MVP launch only if real content exists by then (per `05_CONTENT_STRATEGY.md` §2a/§2b); otherwise they are logged as an explicit, founder-acknowledged fast-follow, not silently dropped from scope.
2. The admin panel supports the full content lifecycle (draft → approve → publish → version history, per the hybrid workflow in `09_CMS_ARCHITECTURE.md` §3) for Case Studies, Builds, Team Members, Testimonials, Blog Posts, Labs/R&D Projects, and Blueprints, under the Head Admin / Admin / Teammate RBAC model.
