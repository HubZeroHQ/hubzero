...
> **Status: Founder Approved — 2026-07-01.** This roadmap was revised to add a real timeline and resourcing model, absent from the original draft — see `00_FOUNDER_APPROVAL.md` §1 and §3 for the full rationale.
...
- Implement the schemas in `11_DATABASE_ARCHITECTURE.md`, including the responsibility-based `User.role` model and `LabsProject` collection (both amended 2026-07-01).
...
3. **Services overview + Software + Hardware** — particularly the Hardware & Embedded page, which has no legacy content to draw from and needs real authorship from Salsabeel's domain knowledge, plus the **Labs / R&D** section as interim proof (`06_PAGE_SPECIFICATIONS.md`, `00_FOUNDER_APPROVAL.md` §2).
...
## Definition of done for "HubZero v2 MVP is launched" **[Amended 2026-07-01]**

1. Every page in `03_INFORMATION_ARCHITECTURE.md` §1 exists, is reachable, and contains real (not placeholder) content, including the Labs/R&D section on the Hardware & Embedded page and 3-5 cornerstone blog articles.
2. The admin panel supports the full content lifecycle (draft → approve → publish → version history, per the hybrid workflow in `09_CMS_ARCHITECTURE.md` §3) for Case Studies, Team Members, Testimonials, Blog Posts, and Labs/R&D Projects, under the Head Admin / Admin / Teammate RBAC model.
