...
>
> Decision convention: see `01_PRODUCT_VISION.md` §0. Nothing below relitigates a decision already settled in `00`–`16` (founding year, RBAC model, CMS build approach, Notes MVP scope, etc.) — this document is additive: it names the operating structure those decisions already sit inside, and gives the four pillars a canonical home so `03`, `05`, `06`, `09`, `11`, and `14` have one place to point back to instead of each inventing their own framing.
...
- **Marketing/community content, recruiting content, thought-leadership writing** → none of the four pillars; this is Notes and Careers territory, unaffected by this document.
...
1. ~~Is the six-item nav ceiling (§5) an acceptable evolution of `03` §2's original five-item rule?~~ **Resolved:** yes — six items, content-gated rollout, Notes moved to footer-only. See `00_FOUNDER_APPROVAL.md` §8, `03_INFORMATION_ARCHITECTURE.md` §2.
