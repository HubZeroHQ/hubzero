...
>
> **Amended.** §12's iconography section is deliberately conceptual and has no bearing on implementation without a concrete icon set — `14_VISUAL_TOKENS.md` §6 specifies the actual minimum icon set, construction rule, and per-icon accessible names this section describes only in principle. §9's diagram conventions gain an authoring-mechanism answer this document never addressed — `15_DIAGRAM_SYSTEM.md` distinguishes CMS-editable editorial diagrams from hand-built engineering diagrams. §5, §9 gain accessibility requirements (legibility with color removed, real text alternatives) in `12_ACCESSIBILITY.md` §2–3. Every reference to "copper" and "drafting cyan" below (§5–6, §9, §11) should be read as **Brand Blue** and **Ice Blue** respectively — `11_COLOR_PHILOSOPHY_AMENDMENT.md` re-derives HubZero's existing blue from the studio's own mark rather than replacing it with an unrelated hue; the two colors' jobs (interactive signal vs. technical-diagram register) are unchanged, only their names and hue family are.
