...
>
> **Amended.** §1, §3 corner-radius and border-weight language ("a small, consistent corner radius," "a thin hairline border") gains actual numbers in `14_VISUAL_TOKENS.md` §3–4. §15 (Architecture diagrams), this document's own "single most important component," gains an authoring-mechanism answer in `15_DIAGRAM_SYSTEM.md` — specifically, which diagrams belong here (hand-built, code-authored) versus in a new CMS block type. §17's deferred interaction-state gap (no hover/focus/active/disabled states specified per component) is partially closed by `12_ACCESSIBILITY.md` §5's focus-state requirements; exact per-component states remain implementation work. §15's "drafting cyan" reference should be read as **Ice Blue**, per `02_VISUAL_LANGUAGE.md`'s amended pointer and `11_COLOR_PHILOSOPHY_AMENDMENT.md` §6.
