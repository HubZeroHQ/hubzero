...
**Status:** Proposed — not yet approved, not yet implemented. No UI exists against this document.
...
**Open question, flagged rather than silently decided:** what happens when an Admin edits an already-**Published** entry — does the edit go live immediately, or does it require re-review? `PLANNING.md` §28 defines the five states but not this specific case. Recommended default: minor edits to a Published entry save and go live immediately (no new reference ID, no slug change, no re-review) unless the team wants a stricter "Published edits re-enter In Review" policy — but this should get an explicit answer before Phase 2 build, not be assumed. Added to the open-decisions list in Appendix B.
...
- **Replacement** — "Replace image" on an inserted block swaps the reference without touching the block's `id` or position, preserving document structure. If the current asset is used elsewhere (per the "used in" list), replacement should insert a *new* Media record rather than mutating the shared one in place — otherwise replacing an image in one Build's technical doc could silently change a hero image on an unrelated Work entry. Flagged as a decision worth confirming explicitly (Appendix B) rather than assumed, since `PLANNING.md` doesn't currently specify this edge case.
...
## Appendix B — Open Decisions Requiring Sign-off

Following the same pattern `PLANNING.md` §40 already uses for the data-model layer — these are product-layer decisions this document takes a position on but that should be explicitly confirmed before Phase 2 build starts, not silently assumed:

1. **Editing a Published entry** — does a minor edit go live immediately, or force re-review? (§5 recommends: goes live immediately, no re-review, for minor edits.)
2. **Replacing a shared Media asset** — does replacement always create a new record when the asset is used elsewhere, or is in-place replacement ever acceptable? (§6 recommends: always create a new record when in use elsewhere.)
3. **Reviewer rejection notes** — confirm a short note on In Review → Draft is worth the one extra field (§5 recommends yes).
4. **Team Member read-visibility scope** — confirm "see everything in Content/Studio, edit only your own/assigned" as the default transparency posture (§8), rather than a narrower per-entry visibility model.
5. **Reusable blocks** — confirm the v1 scope is copy/duplicate only, with true synced reusable blocks deferred alongside version history/collaboration (§36), not built now.

---

**Both the navigation/IA (§1–2), the core experiences (§3–7), and the permissions model (§8) are proposed for review here.** Nothing in this document has been implemented. Once approved — including the five open decisions in Appendix B — this becomes the blueprint Phase 2 (`PLANNING.md` §38) builds against.
